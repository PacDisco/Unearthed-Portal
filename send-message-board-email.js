...
//     url:         "https://portal.unearthededucation.org/index.html" // optional
//   }
//
...
//                       (e.g. info@unearthededucation.org).
//   SMTP_PASS         — Google App Password for that mailbox.
//
// Optional env vars:
//   SMTP_FROM_NAME             — sender display name. Default "Unearthed Education".
//   PORTAL_BASE_URL            — base URL for the "Open the portal" button.
//                                Default https://portal.unearthededucation.org
...
// The program name lives on the associated "Programs"
...
export async function handler(event) {
  try {
    ...
    // Portal objects have different legacy fields (some have
    ...
    // Optional: follow the association from the Portal to a separate
    // "Programs" object and read its name. Unearthed keeps the trip title
    // on the Portal record itself (portal_title), so this stays off unless
    // PROGRAM_OBJECT_TYPE is set. Skipped if we already have a name.
...
function buildPlainText({ firstName, tripTitle, messageHtml, author, link }) {
  ...
  lines.push(`Open the portal: ${link}`, ``, `— Unearthed Education`);
  return lines.join("\n");
}
    ...
    <tr><td align="center" style="padding:32px 16px;">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px; width:100%; background:#ffffff; border:1px solid #e5e7eb;">
        ...
        <tr><td style="padding:0 32px 28px; font-size:12px; line-height:1.6; color:#888; border-top:1px solid #e5e7eb; padding-top:16px;">
          <p style="margin:14px 0 0;">You're receiving this because you're linked to this trip in the Unearthed Education portal.</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
