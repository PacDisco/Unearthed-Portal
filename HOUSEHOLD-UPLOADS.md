...
**Unchanged.** `?email=` still resolves through `authenticateSelf`: your own address, unless you're an admin.
...
The Document Uploads tab is headed UPLOADED DOCUMENTS (not YOUR UPLOADED DOCUMENTS — the list is no longer only yours), carries an "Includes documents uploaded by …" line when someone else is in the pool, and each card reads "Uploaded by you · 14 Feb 2026" or "Uploaded by Peter Reynolds · …".
...
| `public/index.html` | `renderDocumentCard` shows who uploaded each file; the Documents tab states whose uploads the list covers. |
